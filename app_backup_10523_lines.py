...
def send_otp_email(user, otp_code):
    ...
    try:
        msg = Message(
            subject='MAKOKHA MEDICAL CENTRE - Verify Your Email',
            sender=app.config.get('MAIL_USERNAME'),
            recipients=[user.email],
            html=render_template('email/otp_verification.html', user=user, otp_code=otp_code)
        )
        mail.send(msg)
        return True
    except Exception as e:
        logging.error(f"Failed to send OTP email to {user.username}: {e}")
        return False
...
def send_welcome_email(user):
    ...
    try:
        msg = Message(
            subject='Welcome to MAKOKHA MEDICAL CENTRE',
            sender=app.config.get('MAIL_USERNAME'),
            recipients=[user.email],
        ...
        logging.error(f"Failed to send welcome email to {user.username}: {e}")
        return False


# ── Appointment email helpers ───────────────────────────────────────────────

def send_appointment_booked_unpaid_email(appointment, payment):
    """Send email when appointment is booked but payment is still pending."""
    try:
        patient_user = appointment.patient.user
        doctor_user = appointment.doctor.user
        msg = Message(
            subject='MAKOKHA MEDICAL CENTRE - Appointment Booked – Payment Pending',
            sender=app.config.get('MAIL_USERNAME'),
            recipients=[patient_user.email],
            html=render_template('email/appointment_booked_unpaid.html',
                                 user=patient_user, appointment=appointment,
                                 doctor=appointment.doctor, doctor_user=doctor_user,
                                 payment=payment)
        )
        mail.send(msg)
        return True
    except Exception as e:
        logging.error(f"Failed to send unpaid appointment email: {e}")
        return False


def send_payment_received_email(appointment, payment):
    """Send confirmation + invoice email after payment is completed."""
    try:
        patient_user = appointment.patient.user
        doctor_user = appointment.doctor.user
        msg = Message(
            subject='MAKOKHA MEDICAL CENTRE - Payment Confirmed – Appointment Invoice',
            sender=app.config.get('MAIL_USERNAME'),
            recipients=[patient_user.email],
            html=render_template('email/payment_received.html',
                                 user=patient_user, appointment=appointment,
                                 doctor=appointment.doctor, doctor_user=doctor_user,
                                 payment=payment)
        )
        ...
        logging.error(f"Failed to send payment received email: {e}")
        return False
...
def send_appointment_reminder_email(appointment):
    ...
    try:
        ...
        doctor_user = appointment.doctor.user
        msg = Message(
            subject='MAKOKHA MEDICAL CENTRE - Appointment Reminder for Today',
            sender=app.config.get('MAIL_USERNAME'),
            recipients=[patient_user.email],
        ...
        logging.error(f"Failed to send appointment reminder email: {e}")
        return False


def send_appointment_outcome_email(appointment, outcome):
    """Send outcome email after appointment completes, cancels, misses, or is rescheduled.
    outcome: 'completed', 'cancelled', 'missed', 'rescheduled'
    """
    try:
        patient_user = appointment.patient.user
        doctor_user = appointment.doctor.user
        subject_map = {
            'completed': 'Consultation Completed – Thank You!',
            'cancelled': 'Appointment Cancelled',
            'missed': 'Missed Appointment',
            'rescheduled': 'Appointment Rescheduled',
        }
        subject = f"MAKOKHA MEDICAL CENTRE - {subject_map.get(outcome, 'Appointment Update')}"
        msg = Message(
            subject=subject,
            sender=app.config.get('MAIL_USERNAME'),
            recipients=[patient_user.email],
            html=render_template('email/appointment_outcome.html',
                                 user=patient_user, appointment=appointment,
                                 doctor=appointment.doctor, doctor_user=doctor_user,
                                 outcome=outcome)
        )
        ...
        logging.error(f"Failed to send appointment outcome ({outcome}) email: {e}")
        return False


def check_licence_expiry():
    """Check all healthcare practitioners for expired licences and auto-deactivate."""
    with app.app_context():
        today = datetime.now(timezone.utc).date()
        expired_doctors = Doctor.query.filter(
            Doctor.licence_expiry_date != None,
            Doctor.licence_expiry_date < today,
            Doctor.licence_renewal_status != 'expired'
        ).all()
        for doc in expired_doctors:
            doc.licence_renewal_status = 'expired'
            if doc.user:
                doc.user.is_active = False
                logging.warning(f"Auto-deactivated user {doc.user.username} (ID {doc.user.id}) — licence expired {doc.licence_expiry_date}")
        if expired_doctors:
            db.session.commit()
            logging.info(f"Licence expiry check: {len(expired_doctors)} practitioner(s) deactivated")
...
def configure_app():
    ...
    # Database configuration — DATABASE_URL is required (no fallback)
    ...
    app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
        ...
        'poolclass': QueuePool,
        'echo': False,
        # Keep TCP connections alive through Neon sleep / NAT timeouts
        'connect_args': {
            'keepalives': 1,
            'keepalives_idle': 30,
...
def _is_database_seeded():
    """Check if seed users have already been created (one-time guard).
    Uses a lightweight marker table so seeding never runs twice."""
    try:
        insp = inspect(db.engine)
        if '_seed_status' in insp.get_table_names():
            with db.engine.connect() as conn:
                row = conn.execute(text("SELECT seeded FROM _seed_status LIMIT 1")).fetchone()
...
def _mark_database_seeded():
    """Record that seeding has completed so it never reruns."""
    try:
        with db.engine.begin() as conn:
            conn.execute(text(
                "CREATE TABLE IF NOT EXISTS _seed_status (seeded BOOLEAN NOT NULL DEFAULT TRUE)"
            ))
            conn.execute(text("DELETE FROM _seed_status"))
            conn.execute(text("INSERT INTO _seed_status (seeded) VALUES (TRUE)"))
    except Exception as e:
        print(f'✗ Failed to mark database as seeded: {e}')


def _parse_seed_users():
    """Parse seed user definitions from environment variables.

    Expected env vars (all pipe-separated fields):
      SEED_ADMIN   = username|email|password|first_name|last_name|phone
      SEED_DOCTOR_1 .. SEED_DOCTOR_3 = username|email|password|first_name|last_name|phone|specialization|license_number|experience_years|consultation_fee|qualifications|practitioner_type
      SEED_PATIENT_1 .. SEED_PATIENT_4 = username|email|password|first_name|last_name|phone|blood_type|emergency_contact|insurance_provider
    """
    users = []

    # --- Admin ---------------------------------------------------------
    admin_raw = os.getenv('SEED_ADMIN')
    if admin_raw:
        parts = [p.strip() for p in admin_raw.split('|')]
        if len(parts) >= 6:
            users.append({
                'role': 'admin',
                'username': parts[0], 'email': parts[1], 'password': parts[2],
                'first_name': parts[3], 'last_name': parts[4], 'phone': parts[5],
            })

    # --- Doctors (up to 3) ---------------------------------------------
    for i in range(1, 4):
        doc_raw = os.getenv(f'SEED_DOCTOR_{i}')
        if not doc_raw:
            continue
        parts = [p.strip() for p in doc_raw.split('|')]
        if len(parts) >= 6:
            entry = {
                'role': 'doctor',
                'username': parts[0], 'email': parts[1], 'password': parts[2],
            ...
            if len(parts) > 6:  entry['specialization'] = parts[6]
            if len(parts) > 7:  entry['license_number'] = parts[7]
            if len(parts) > 8:  entry['experience_years'] = parts[8]
            if len(parts) > 9:  entry['consultation_fee'] = parts[9]
            if len(parts) > 10: entry['qualifications'] = parts[10]
            if len(parts) > 11: entry['practitioner_type'] = parts[11]
            users.append(entry)

    # --- Patients (up to 4) --------------------------------------------
    for i in range(1, 5):
        pat_raw = os.getenv(f'SEED_PATIENT_{i}')
        if not pat_raw:
            continue
        parts = [p.strip() for p in pat_raw.split('|')]
        if len(parts) >= 6:
            entry = {
                'role': 'patient',
                'username': parts[0], 'email': parts[1], 'password': parts[2],
            ...
            if len(parts) > 6: entry['blood_type'] = parts[6]
            if len(parts) > 7: entry['emergency_contact'] = parts[7]
            if len(parts) > 8: entry['insurance_provider'] = parts[8]
            users.append(entry)
...
def seed_users():
    """One-time database seeding: creates users defined in .env variables.
    Skips entirely if seeding was already performed (checked via _seed_status table)."""
    with app.app_context():
        if _is_database_seeded():
            print('✓ Database already seeded — skipping user creation')
            return

        seed_data = _parse_seed_users()
        if not seed_data:
            print('⚠ No SEED_* environment variables found — skipping user seeding')
            return

        created = 0
        for entry in seed_data:
            # Skip if username already exists (safety)
            if User.query.filter_by(username=entry['username']).first():
                print(f"✓ Seed user '{entry['username']}' already exists")
                continue
            ...
            if entry['role'] == 'doctor':
                doc = Doctor(
                    user_id=user.id,
                    specialization=entry.get('specialization', 'General Practitioner'),
                    license_number=entry.get('license_number', ''),
                    experience_years=int(entry.get('experience_years', 0) or 0),
                    consultation_fee=float(entry.get('consultation_fee', 0) or 0),
                    qualifications=entry.get('qualifications', ''),
                    practitioner_type=entry.get('practitioner_type', ''),
                    availability=True,
                )
                db.session.add(doc)
                ...
                pat = Patient(
                    ...
                    emergency_contact=entry.get('emergency_contact', ''),
                    insurance_provider=entry.get('insurance_provider', ''),
                )
                db.session.add(pat)

            print(f"✓ Seed {entry['role']} '{entry['username']}' created")
            created += 1

        try:
            db.session.commit()
            _mark_database_seeded()
            print(f'✓ Database seeding complete — {created} user(s) created')
        except Exception as e:
            db.session.rollback()
...
def initialize_database():
    ...
    with app.app_context():
        try:
            ...
            # Ensure doctors table has new profile columns
            try:
                if 'doctors' in insp.get_table_names():
                    doc_cols = [c['name'] for c in insp.get_columns('doctors')]
                    doc_needed = {
                        'licence_regulatory_body': 'VARCHAR(200)',
                        'licence_issue_date': 'DATE',
                        'licence_expiry_date': 'DATE',
                        'licence_renewal_status': 'VARCHAR(30)',
                        'practitioner_type': 'VARCHAR(100)',
                        'encrypted_awards': 'BYTEA',
                    }
                    for col, coltype in doc_needed.items():
            ...
            # Ensure appointments table has reminder_sent column
            try:
                if 'appointments' in insp.get_table_names():
                    apt_cols = [c['name'] for c in insp.get_columns('appointments')]
                    if 'reminder_sent' not in apt_cols:
                        try:
                            with db.engine.begin() as conn:
                                conn.execute(text("ALTER TABLE appointments ADD COLUMN reminder_sent BOOLEAN DEFAULT FALSE"))
                            print('✓ Added missing column appointments.reminder_sent')
                        except Exception as e:
                            print(f'✗ Failed to add appointments.reminder_sent:', e)
            except Exception as e:
                print('✗ Failed to inspect appointments table:', e)
            
            # One-time seed users from .env (skips if already seeded)
            seed_users()

            # Check licence expiry and auto-deactivate expired practitioners
            try:
                check_licence_expiry()
            except Exception as e:
                print(f'✗ Licence expiry check failed: {e}')
                ...
                if apt:
                    ...
                    try:
                        ...
                        if doctor_user_id and ended_by and int(ended_by) == int(doctor_user_id):
                            ...
                            # Send completion email + review request
                            try:
                                send_appointment_outcome_email(apt, 'completed')
                            except Exception:
                                pass
                    except Exception:
                        pass
...
def appointment_reminder_worker(interval_seconds=600):
    """Background worker that sends day-of-appointment reminder emails."""
    ...
    while True:
        try:
            with app.app_context():
                ...
                # Find confirmed/pending appointments for today that haven't been reminded
                ...
                for apt in due:
                    try:
                        send_appointment_reminder_email(apt)
                        apt.reminder_sent = True
                    except Exception as e:
                        logging.warning(f"Reminder email failed for appointment {apt.id}: {e}")
                if due:
                    db.session.commit()
        except Exception:
            try:
...
def start_background_workers():
    ...
    try:
        t2 = threading.Thread(target=appointment_reminder_worker, kwargs={'interval_seconds':600}, daemon=True, name='appointment_reminder_worker')
        t2.start()
        app.logger.info('Started background thread: appointment_reminder_worker')
    except Exception as e:
        try:
            app.logger.exception('Failed to start appointment_reminder_worker: %s', e)
        except Exception:
            print(f'Failed to start appointment_reminder_worker: {e}')
...
def inject_role_helpers():
    """Inject role display helpers into all templates"""
    role_labels = {
        'admin': 'Admin',
        'doctor': 'Healthcare Practitioner',
        'non_staff': 'Non-Staff Worker',
        'patient': 'Patient',
    }
    def role_display(role):
        return role_labels.get(role, role or '')
    return dict(role_display=role_display)
...
def login():
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            if user.email_verified is False and user.role != 'patient':
                flash('Please verify your email before logging in.', 'warning')
                return redirect(url_for('verify_email', user_id=user.id))
            if user.is_active:
                # Check licence expiry for healthcare practitioners
                if user.role == 'doctor' and user.doctor_profile:
                    doc = user.doctor_profile
                    if doc.licence_expiry_date and doc.licence_expiry_date < datetime.now(timezone.utc).date():
                        doc.licence_renewal_status = 'expired'
                        user.is_active = False
                        db.session.commit()
                        flash('Your practising licence has expired. Your account has been deactivated. Please contact administration.', 'error')
                        return render_template('auth/login.html')
        ...
        try:
            ...
            if pic and pic.filename and allowed_file(pic.filename):
                try:
                    raw = pic.read()
                    encrypted_bytes = encrypt_file_bytes(raw)
                    user.profile_picture_blob = encrypted_bytes
                    user.profile_picture_mime = pic.content_type or 'image/jpeg'
                    user.profile_picture_name = secure_filename(pic.filename)
                    db.session.commit()
                except Exception as pic_err:
                    logging.warning(f"Profile picture upload failed during signup for {user.username}: {pic_err}")
...
def verify_email(user_id):
    ...
    if request.method == 'POST':
        ...
        # Find valid OTP
        otp_record = EmailOTP.query.filter_by(user_id=user.id, used=False).order_by(EmailOTP.created_at.desc()).first()
        if otp_record and otp_record.is_valid(otp_code):
...
def resend_otp(user_id):
    ...
    recent = EmailOTP.query.filter_by(user_id=user.id, used=False).order_by(EmailOTP.created_at.desc()).first()
    if recent and (datetime.now(timezone.utc) - recent.created_at).total_seconds() < 120:
        msg = 'Please wait before requesting a new OTP.'
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
...
@app.route('/admin/resend_otp/<int:user_id>', methods=['POST'])
@login_required
def admin_resend_otp(user_id):
    """Admin can resend OTP for any user"""
    if current_user.role != 'admin':
...
def add_user():
    ...
    # Validate role — patient cannot be created here
    valid_roles = ('admin', 'doctor', 'non_staff')
    if role not in valid_roles:
        msg = f'Invalid role. Allowed: {", ".join(valid_roles)}'
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
    ...
    try:
        ...
        if role == 'doctor':
            ...
            # Set doctor fields from form data
            if data.get('practitioner_type'):
                doctor.practitioner_type = data.get('practitioner_type')
            ...
            if data.get('qualifications'):
                doctor.qualifications = data.get('qualifications')
            if data.get('awards'):
                doctor.awards = data.get('awards')
            if data.get('consultation_fee'):
                try:
                    doctor.consultation_fee = float(data.get('consultation_fee'))
                except (ValueError, TypeError):
                    pass
            if data.get('experience_years'):
                try:
                    doctor.experience_years = int(data.get('experience_years'))
                except (ValueError, TypeError):
                    pass
            if data.get('licence_regulatory_body'):
                doctor.licence_regulatory_body = data.get('licence_regulatory_body')
            if data.get('licence_issue_date'):
                try:
                    doctor.licence_issue_date = datetime.strptime(data.get('licence_issue_date'), '%Y-%m-%d').date()
                except (ValueError, TypeError):
                    pass
            if data.get('licence_expiry_date'):
                try:
                    doctor.licence_expiry_date = datetime.strptime(data.get('licence_expiry_date'), '%Y-%m-%d').date()
                except (ValueError, TypeError):
                    pass
            if data.get('licence_renewal_status'):
                doctor.licence_renewal_status = data.get('licence_renewal_status')
        ...
        if pic and pic.filename and allowed_file(pic.filename):
            try:
                ...
                logging.warning(f"Profile picture upload failed for new user {user.username}: {pic_err}")
    ...
    otp_msg = ' OTP verification email sent.' if email_sent else ' (Email sending failed — verify manually)'
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return jsonify({'success': True, 'user_id': user.id, 'message': f'User created.{otp_msg}'})

    flash(f'User added successfully.{otp_msg}', 'success')
    return redirect(url_for('admin_users'))
...
def admin_get_user(user_id):
    ...
    try:
        result = {
            ...
            'preferred_payment_method': u.preferred_payment_method or '',
            'department': u.department or '',
            'job_category': u.job_category or '',
            'profile_picture': bool(u.profile_picture),
            'created_at': u.created_at.isoformat() if u.created_at else None,
        }
        # Doctor-specific fields
        if u.role == 'doctor' and u.doctor_profile:
            d = u.doctor_profile
            result['specialization'] = d.specialization or ''
            result['license_number'] = d.license_number or ''
            result['qualifications'] = d.qualifications or ''
            result['experience_years'] = d.experience_years
            result['consultation_fee'] = d.consultation_fee
            result['practitioner_type'] = d.practitioner_type or ''
            result['awards'] = d.awards or ''
            result['licence_regulatory_body'] = d.licence_regulatory_body or ''
            result['licence_issue_date'] = d.licence_issue_date.isoformat() if d.licence_issue_date else ''
            result['licence_expiry_date'] = d.licence_expiry_date.isoformat() if d.licence_expiry_date else ''
            result['licence_renewal_status'] = d.licence_renewal_status or ''
    ...
    for u in users:
        try:
            entry = {
                ...
                'job_category': u.job_category or '',
                'created_at': u.created_at.isoformat() if u.created_at else None,
                'has_profile_picture': bool(u.profile_picture_blob or getattr(u, 'encrypted_profile_picture_path', None))
            }
            # Add doctor extras for table display
            if u.role == 'doctor' and u.doctor_profile:
                d = u.doctor_profile
                entry['specialization'] = d.specialization or ''
                entry['practitioner_type'] = d.practitioner_type or ''
                entry['licence_renewal_status'] = d.licence_renewal_status or ''
            out.append(entry)
        except Exception:
...
def admin_update_user():
    ...
    try:
        ...
        # New shared fields
        if 'title' in data:
            u.title = data.get('title') or None
        if 'email_verified' in data:
            u.email_verified = data.get('email_verified') in ('true', 'True', '1', True, 1)
        if 'bank_account' in data:
            u.bank_account = data.get('bank_account') or None
        if 'bank_name' in data:
            u.bank_name = data.get('bank_name') or None
        if 'preferred_payment_method' in data:
            u.preferred_payment_method = data.get('preferred_payment_method') or None
        if 'department' in data:
            u.department = data.get('department') or None
        if 'job_category' in data:
            u.job_category = data.get('job_category') or None
        ...
        if role == 'doctor':
            ...
            if doc:
                if 'specialization' in data:
                    doc.specialization = data.get('specialization') or None
                if 'license_number' in data:
                    doc.license_number = data.get('license_number') or None
                if 'qualifications' in data:
                    doc.qualifications = data.get('qualifications') or None
                if 'consultation_fee' in data:
                    fee = data.get('consultation_fee')
                    doc.consultation_fee = float(fee) if fee else None
                if 'experience_years' in data:
                    yrs = data.get('experience_years')
                    doc.experience_years = int(yrs) if yrs else None
                if 'practitioner_type' in data:
                    doc.practitioner_type = data.get('practitioner_type') or None
                if 'awards' in data:
                    doc.awards = data.get('awards') or None
                if 'licence_regulatory_body' in data:
                    doc.licence_regulatory_body = data.get('licence_regulatory_body') or None
                if 'licence_issue_date' in data:
                    val = data.get('licence_issue_date')
                    doc.licence_issue_date = datetime.strptime(val, '%Y-%m-%d').date() if val else None
                if 'licence_expiry_date' in data:
                    val = data.get('licence_expiry_date')
                    doc.licence_expiry_date = datetime.strptime(val, '%Y-%m-%d').date() if val else None
                if 'licence_renewal_status' in data:
                    doc.licence_renewal_status = data.get('licence_renewal_status') or None
                db.session.add(doc)
                db.session.commit()
        ...
        if pic and pic.filename and allowed_file(pic.filename):
            try:
                raw = pic.read()
                encrypted_bytes = encrypt_file_bytes(raw)
                u.profile_picture_blob = encrypted_bytes
                u.profile_picture_mime = pic.content_type or 'image/jpeg'
                u.profile_picture_name = secure_filename(pic.filename)
                db.session.commit()
            except Exception as pic_err:
                logging.warning(f"Profile picture update failed for user {u.username}: {pic_err}")
...
def cancel_appointment(appointment_id):
    ...
    try:
        ...
        try:
            send_appointment_outcome_email(appointment, 'cancelled')
        except Exception as email_err:
            logging.warning(f"Cancellation email failed: {email_err}")
...
def payment_confirm(payment_id):
    ...
    try:
        ...
        try:
            if appointment:
                send_payment_received_email(appointment, payment)
        except Exception as email_err:
            logging.warning(f"Payment confirmation email failed: {email_err}")
...
def book_appointment():
    ...
    try:
        ...
        try:
            ...
            # Send unpaid appointment notification email
            try:
                send_appointment_booked_unpaid_email(appointment, payment)
            except Exception as email_err:
                logging.warning(f"Appointment booked email failed: {email_err}")
...
def get_testimonials():
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error('Error in get_testimonials: %s', e)
        return jsonify({'testimonials': [], 'total': 0, 'page': 1, 'per_page': 10, 'doctor_average': None})
