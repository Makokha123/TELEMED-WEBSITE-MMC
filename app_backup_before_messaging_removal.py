...
@app.route('/admin/api/appointment/<int:appointment_id>/messages')
@login_required
def admin_api_appointment_messages(appointment_id):
    """Get all messages for a specific appointment (admin monitoring)."""
    if current_user.role != 'admin':
        return jsonify({'error': 'Access denied'}), 403
    # Get from Communication model
    comms = Communication.query.filter_by(appointment_id=appointment_id).order_by(Communication.timestamp.asc()).limit(500).all()
    messages = []
    for c in comms:
        sender = db.session.get(User, c.sender_id)
        messages.append({
            'id': c.id,
            ...
            'message_type': c.message_type or 'text',
            'timestamp': c.timestamp.isoformat() if c.timestamp else None,
            'status': getattr(c, 'message_status', 'sent'),
        })
...
def get_appointment_messages_enhanced(appointment_id):
    ...
    try:
        ...
        total_query = query
        messages = query.order_by(Communication.timestamp.desc(), Communication.id.desc()).offset((page - 1) * per_page).limit(per_page).all()
        messages = list(reversed(messages))
...
@app.route('/video-call/room/<int:appointment_id>')
@login_required
def video_call_room_redirect(appointment_id):
    return redirect(url_for('consultation_room', appointment_id=appointment_id), 301)
...
def incoming_video_call(appointment_id):
    """Redirects to consultation room (legacy incoming call page no longer used)"""
    return redirect(url_for('consultation_room', appointment_id=appointment_id), 302)
...
@app.route('/video-call-legacy/<int:appointment_id>')
@login_required
def video_call_legacy(appointment_id):
    """Video call page for appointments (legacy fallback)"""""
    appointment = Appointment.query.get_or_404(appointment_id)
    observe_mode = request.args.get('observer') == '1' and current_user.role == 'admin'
...
@app.route('/voice-call-legacy/<int:appointment_id>')
@login_required
def voice_call_legacy(appointment_id):
    """Voice call page for appointments (legacy fallback)"""
    appointment = Appointment.query.get_or_404(appointment_id)
    observe_mode = request.args.get('observer') == '1' and current_user.role == 'admin'
...
def outgoing_call(appointment_id):
    """Redirects to consultation room (no more outgoing ringing flow)"""
    return redirect(url_for('consultation_room', appointment_id=appointment_id), 302)
...
def handle_message_delivered(data):
    ...
    if message and message.appointment_id == appointment_id:
        ...
        emit('message_status_updated', {
            ...
            'status': message.message_status or 'delivered',
            'appointment_id': appointment_id,
            'receipt_data': receipts
        }, room=f'appointment_{appointment_id}')
...
def handle_send_message_enhanced(data):
    ...
    try:
        ...
        if not _rate_limit(current_user.id, 'send_message', 30, 60):
            emit('message_error', {'error': 'Rate limit exceeded', 'client_msg_id': client_msg_id})
        ...
        prescription_id = data.get('prescription_id') if isinstance(data, dict) else None
        ...
        if not appointment_id or (not content and message_type == 'text'):
            emit('message_error', {'error': 'Invalid data', 'client_msg_id': client_msg_id})
            return False

        appointment = db.session.get(Appointment, appointment_id)
        if not appointment:
            emit('message_error', {'error': 'Appointment not found', 'client_msg_id': client_msg_id})
            return False

        # Verify access
        if not verify_appointment_access(appointment, current_user):
            emit('message_error', {'error': 'Access denied', 'client_msg_id': client_msg_id})
            return False
        ...
        if current_user.role == 'patient':
            can_access, reason = can_patient_access_messaging(appointment_id, current_user)
            if not can_access:
                emit('message_error', {'error': reason, 'error_type': 'payment_required', 'client_msg_id': client_msg_id})
        ...
        # Prepare reply-to preview for broadcast
        reply_to_obj = None
        if communication.reply_to_message_id:
            parent = db.session.get(Communication, communication.reply_to_message_id)
            if parent:
                reply_to_obj = {
                    'id': parent.id,
                    'sender_name': safe_display_name(parent.sender) if parent.sender else 'Unknown',
                    'content': (parent.content or '')[:120]
                }
        ...
        # Mark as delivered immediately when recipient is currently in appointment room
        room_member_ids = _room_member_ids(appointment_room)
        recipient_in_room = bool(recipient_id and int(recipient_id) in set(room_member_ids))

        # Fallback to online status if room membership is unavailable
        if recipient_id and (recipient_in_room or _is_user_online(recipient_id)):
            communication.message_status = 'delivered'
            db.session.commit()
        ...
        import traceback
        traceback.print_exc()
        fallback_client_msg_id = data.get('client_msg_id') if isinstance(data, dict) else None
        emit('message_error', {'error': str(e), 'client_msg_id': fallback_client_msg_id})
...
def handle_message_read(data):
    ...
    try:
        ...
        if message_id:
            ...
            db.session.commit()
            emit('messages_read', {'appointment_id': appointment_id, 'messages': updated}, room=f'appointment_{appointment_id}')
    except Exception as e:
        db.session.rollback()
...
@socketio.on('voice_recording_status')
def handle_voice_recording_status(data):
    """Broadcast live voice-note recording status to other participants in appointment room."""
    if not current_user.is_authenticated:
        return
    ...
    is_recording = bool((data or {}).get('is_recording'))
    appointment_room = f'appointment_{appointment_id}'

    emit('voice_recording_status', {
        'user_id': current_user.id,
        'user_name': safe_display_name(current_user),
        'appointment_id': int(appointment_id),
...
def handle_join_appointment(data):
    ...
    try:
        ...
        delivered_updates = []
        pending_messages = Communication.query.filter_by(appointment_id=appointment_id).filter(
            Communication.sender_id != current_user.id,
            Communication.message_status == 'sent'
        ).order_by(Communication.timestamp.desc(), Communication.id.desc()).limit(200).all()
        for message in pending_messages:
            receipts = _record_message_receipt(message, current_user.id, 'delivered')
            delivered_updates.append({
    ...
    try:
        ...
        if _is_user_online(callee_user_id):
            ...
            def voice_call_timeout():
                try:
                    with app.app_context():
                        # Check DB first — if already ended, skip (race with user end_voice_call)
                        ...
                        if current and current.get('status') == 'ringing':
                            ...
                            _emit_call_lifecycle(current, 'missed', extra={'reason': 'unanswered'})
                            _insert_call_event_message(appointment_id, current_user.id, callee_user_id, 'voice', 'missed')
            ...
            socketio.start_background_task(lambda: (socketio.sleep(app.config.get('CALL_RING_TIMEOUT', 60)), voice_call_timeout()))
        else:
                ...
                try:
                    with app.app_context():
                        ...
                        if current and current.get('status') == 'ringing':
                            ...
                            _insert_call_event_message(appointment_id, current_user.id, callee_user_id, 'voice', 'connection_failed')
            ...
            socketio.start_background_task(lambda: (socketio.sleep(app.config.get('CALL_OFFLINE_TIMEOUT', 90)), voice_call_offline_timeout()))
...
def handle_end_voice_call(data):
    ...
    try:
        ...
        # Insert call event into chat timeline
        event_type = 'completed' if reason == 'completed' else ('declined' if reason in ('rejected','declined','callee_declined','user_declined') else ('missed' if reason in ('unanswered','missed') else reason))
        _insert_call_event_message(appointment_id, caller_id, callee_id, 'voice', event_type, duration=duration if duration > 0 else None)
